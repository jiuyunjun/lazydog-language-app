...
### 2.2 贴近读音的中文助记链（D-072，收紧 D-069 的宽松近音）

用户认可的方向是从熟悉的中文关键词回想英文，再通过动作/画面连接当前词义。
谐音以贴近原词真实读音为硬前提，先保读音和当前词义，再追求好记、好笑。
诙谐来自动作、反差和夸张画面，不能为了中文包袱改变英文读音；字形联想也不能冒充词源。

```text
反例：crab → 快来剥，把一个音节拉成三个，故事再好笑也不合格。
反例：ambition → 俺必胜，不能因为画面联系雄心就忽略重读部分和词尾的声音偏差。
字形联想：dessert 中间两个 s 像两份甜点，提醒自己别少写一个 s。
```

- 先按当前词性和词义核对标准音标、音节数、重音和音素顺序，再尝试中文声音关键词。
  不按字母名称或拼写猜发音，不改音标或混拼口音来迁就抓手；拿不准读音就不用谐音。
- 逐音节贴近主要元辅音，尤其重读部分；不增删音节、拆散辅音连缀、调换音序或补词尾。
  普通话没有对应音时仅容许最接近的近似，差异明显就放弃。只借局部近音时标清对应部分，
  不冒充整词谐音，也不强行补齐汉字。不能用生僻汉字或无意义中文串勉强凑音。
- 只用一个短、熟悉的中文抓手，讲清它与当前词义的联系；笑点放在画面，不牺牲声音贴近度。
- 谐音不是实际读音，标「谐音联想」和短注「助记，非读音」。音标、整词朗读、
  pronunciation 字段仍保持真实发音，不能按中文关键词拆英文音节；「非读音」短注不豁免声音偏差。
...
自检必须回答：用了哪段声音/字形？谐音逐音节是否贴近，有无增删音节、丢辅音、换主要元音或添词尾？
是否为了包袱改音？中文抓手怎么连到词义？遮住英文，是否仍有有辨识度的链？
任何明显声音偏差都改用字形/可靠构词或留空。本次仅收紧生成规则，旧谐音不自动重写，仍可用换法入口更新。
...
**策略选择（D-072）**：优先尝试贴近真实读音的中文谐音/字形联想；有实际错拼时针对错误，
...
- 禁止百科式解释
- 禁止编造词源
- 禁止偏离原词读音的硬凑谐音；按 §2.2 逐音节核对后才可使用，不为好笑改音
- 禁止强行联想
...
```json
{
  ...
  "memory_hook": "字形联想：dessert 中间两个 s 像两份甜点，嘴上说吃饱了，手却一份也不肯少拿。",
  "morphology": null,
  "spelling": null,
  "pronunciation": null,
  ...
  "recall_question": "饭后端上蛋糕，这份甜点用刚学的英文怎么说？"
}
```
...
ambition / crab（不得沿用旧偏音谐音）、appear（禁止场景凑数）、bizarre（不得硬造故事）。检查不查辅助词能理解、无假词源、
