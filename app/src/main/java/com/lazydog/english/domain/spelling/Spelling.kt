...
object SpellingEngine {
    ...
    fun evaluate(
        ...
        val nextFailureStreak = if (correct) 0 else progress.failureStreak + 1
        ...
        val nextStage = deriveNextStage(provisional, correct, hintLevel)
    ...
    fun chunkWord(word: String): List<String> {
        ...
        val commonSuffix = listOf("tion", "sion", "ment", "ness", "able", "ible", "ing", "ed", "ly")
            .firstOrNull { clean.length - it.length >= 3 && clean.endsWith(it) }
        if (commonSuffix != null) return listOf(clean.dropLast(commonSuffix.length), commonSuffix)
    ...
    private fun deriveNextStage(progress: SpellingProgress, correct: Boolean, hintLevel: Int): SpellingStage {
