...
- Coil（只用于显示外链缩略图，D-071 批准；用 2.x 不用 3.x，后者会把 OkHttp 4 一起拖着换）
- Hilt（仅在依赖关系复杂到值得引入时）
...
`单词视觉记忆图片DESIGN.md` 的落点（D-071）。核心也是一句话：**图片挂在词义上，不挂在词形上**。

- `vocabulary_sense_images`（Room v21）的主键是 `SenseKey` 而不是 itemId，也不是 term：
  已入库的词是 `item:<itemId>`，还没添加的草稿卡是 `draft:<词|词性|释义>`。按词形缓存的话，
  `charge` 的「充电」和「指控」会共用一张图——这正是整个模块存在的理由。草稿键是稳定的，
  所以预览时找到的图，按下「添加」之后不用重搜。也因为草稿没有 itemId，这张表没有外键，
  不随知识项级联删除。
- 存的是**引用**（缩略图地址、原页面地址、来源站名），不是图本身。Brave 负责发现，
  不等于拿到第三方图片的版权（设计文档 §33、§35），所以不把原图下载到自己的存储里。
  代价是外链会失效，靠留 Top 3 依次顶上兜（§37）。
- **这张表不进备份**：它是会过期的第三方地址缓存，而且 senseKey 里嵌着 itemId，
  备份恢复时 itemId 会重映射，照搬旧键会把一个词的图静默挂到另一个词上。
- 链路全在 `VocabularyImageRepository`：`词义 → 能不能画 → 检索词 → Brave → 本地过滤 → Top 3 → 落库`。
  页面只说"给这个词义找张图"或"这批不行，换一批"。同一个词义同时只跑一条发现流程
  （词卡和阅读预取可能同时想起同一个词），否则是两倍的模型调用和两倍的搜索配额。
- 检索词是**单独一次模型调用**（`generateVisualSearchPlan`），不并进新词生成：
  它的输出是给搜索引擎用的英文查询，和词卡里给人看的中文内容是两种东西，而且它要能
  对着某一个词义单独重来。「这个词义画不出来」是合法结论并且**会落库**，
  否则每次打开 `although` 都会再问一次模型。
- 排序在 `VisualCandidateFilter`：MVP 不跑 Vision（§28），所以只用尺寸、来源和标题做判断，
  不假装在做语义匹配。真正的排序质量靠"用户换了几次图"暴露（`replacedByUser`）。
- UI（`feature/vocabulary/WordImagePanel.kt`）挂在四处，各自不同：新词卡与记录详情用完整面板
  （可换图、可反馈），拼写接触卡和早期题型的答案面、母语阅读点词抽屉用 `CachedSenseImage`
...
- Brave 那一个密钥有两个用途：母语阅读的联网检索（`WebSearchProvider`）和单词配图
  （`ImageSearchProvider`）。设置里那一行叫「网页与图片搜索（Brave）」，
  没配时明说这两个功能都不可用。`单词视觉记忆图片DESIGN.md` §38 要求密钥只放后端，
  这里有意偏离（D-071）：本项目没有后端那一层，密钥是用户自己的。
