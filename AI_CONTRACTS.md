...
### 「怎么记」质量与首次学词（D-068，替代 D-062 的写法和回想展示）

- 批量新词 `memoryHintZh`、点词解释与独立 `memory_hook` 共用规则：20～90 字、最多两句，
  中文讲通一个锚点与目标英文的联系；主提示包含目标词，辅助英文/词缀就地给中文含义。
  不凭 CEFR 假设认识具体辅助词。真实错拼优先，透明构词才拆，否则用一个双语日常短表达。
- 仅一个短语，尽量 2～5 个英文词；不堆近义词或冷僻词根。新词短提示仍用策略标签开头。
  人工联想标明性质；自然贴近的声音关键词须通过中文动作画面连接词义，标「谐音联想，非读音」。
  正反例及研究适用边界见 `词汇记忆提示DESIGN.md` §2.2～§2.3。
- 短提示可留空。本地继续接受 4～90 字，拦确定空话、裸词义重复、无英文或无中文、重复钩子；
  批量不合格提示清空但保留词卡，独立不合格则失败。字符规则不保证语义质量或学习效果。
- `core_meaning` 对齐请求的当前词义，未指定才选常用义。`recall_question` 为 60 字内中文情境，
  要求回想完整英文单词，不只考某个字母或词缀；不出现目标词或拆开的答案，独立展示也能理解。
  本地对问题不做截断，检查空白/长度/中文/直接泄题；无法自动判定所有拆分泄题和语义歧义。
- 首屏提供「遮住，回想一下」，全屏遮住词卡后展示问题，用户点「看答案」再核对；不计分。
...
- 新词与点词提示版本分别为 `WORDS_PROMPT_VERSION=4`、`WORD_EXPLANATION_PROMPT_VERSION=3`，
  独立记忆提示为 `MEMORY_PROMPT_VERSION=3`；输出 schema 仍为 1，旧数据不自动重写。
