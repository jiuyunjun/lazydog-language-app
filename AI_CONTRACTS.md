...
### 「怎么记」质量与首次学词（D-069，保留 D-068 的全屏回想）

- 批量新词、点词解释、独立生成共用中文助记规则：20～90 字、最多两句，
  优先「英文 → 中文声音/字形抓手 → 词义画面」。ambition/俺必胜、crab/快来剥是用户认可的示例。
  允许宽松近音和夸张联想，不要求准确音译；谐音须标明「谐音联想」「助记，非读音」。
...
- 新词与点词提示版本分别为 `WORDS_PROMPT_VERSION=5`、`WORD_EXPLANATION_PROMPT_VERSION=4`，
  独立记忆提示为 `MEMORY_PROMPT_VERSION=4`；输出 schema 仍为 1，旧数据不自动重写。
