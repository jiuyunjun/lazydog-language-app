...
### 2.2 中文母语者的最低理解成本（D-068）

旧方案容易把「多写一点英语」误当成记忆线索：例如「正式场合里的 buy」要求先认识另一个词，
而「想象一个奇怪画面就是 bizarre」只是在重述词义。两者都没有把熟悉的线索与新词连起来。
本次用以下规则替代 D-062 的写法优先级，七类策略和数据结构继续沿用：

- 先用中文讲通一个联系，再嵌入要学的英文。辅助英文词、词缀、短语就地给中文含义；
  不能用 CEFR 等级推断学习者一定认识 buy 或某个词根。无需点查词典才能看懂主提示。
- 只保留一个锚点。主提示包含目标词与当前词义的联系；有真实错拼优先解释正确字母顺序，
  构词透明才拆，不强行加入近义词或词源。
- 没有可靠助记时，用一个中文日常情境引出双语短表达（尽量 2～5 个英文词）；
  它提供用法和回想起点，不宣称凭一句话就能记牢。短提示连用法也写不好时留空。
- 声音关键词可作为人工联想，但须自然贴近，并通过有动作的中文画面连接关键词与词义。
  标明「谐音联想，非读音」；不能只贴谐音，更不能替代音标与实际朗读。
- 看完后提供真正遮住答案的回想，再揭晓核对；没有自动评分、掌握度更新或计时压力。
...
- [WordCraft, 2026, 作者预印本](https://arxiv.org/html/2602.00762v1)：
...
```text
purchase a ticket（购买一张票）：订票页面让你付款买票，用 purchase 表示这次购买。
```
...
谐音不默认生成，也不一概排除；按 §2.2 审查声音贴近程度、中文联系和发音风险，
不因幽默或策略标签而优先。
...
3. 记忆钩子
- 用 20～90 字、最多两句讲通一个中文锚点与目标英文的联系（D-068），包含目标词。
- 辅助英文就地译成中文，不默认已知词根、近义词；没有可靠助记则给双语短表达，详见 §2.2。
...
```json
{
  ...
  "recall_question": "订票页面上，刚学的「购买」怎么说？"
}
```
...
purchase（用法兜底）、bizarre（不得硬造故事）。检查不查辅助词能理解、无假词源、
...
```text
if memory_hook missing or too long or generic advice or no English cue or no Chinese explanation:
    reject
