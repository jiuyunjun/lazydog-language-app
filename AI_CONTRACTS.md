...
提示词版本 `VISUAL_QUERY_PROMPT_VERSION=2`：
...
- 新词与点词提示版本分别为 `WORDS_PROMPT_VERSION=6`、`WORD_EXPLANATION_PROMPT_VERSION=5`，
  独立记忆提示为 `MEMORY_PROMPT_VERSION=5`；输出 schema 仍为 1，旧数据不自动重写。
...
提示词版本 `NATIVE_READING_PROMPT_VERSION=2`（两次调用共用，schema 仍为 1）：
