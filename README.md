...
当前状态：M1（知识库与复习内核）、M5 朗读（提前，见 D-013）、M3（AI 内容与单词/语法学习）代码完成。学习页三个真入口：单词（到期复习 + AI 上新，四档自评入库）、语法（AI 讲解 + 记入知识库）、朗读（TTS 示范 + 发音评估，语速可调）。AI 输出走 json_object + 本地契约校验，不合格不入库；网络失败自动重试一次。今日页与首页 demo 会话仍是示例数据（M6 收口）。下一步：M2（能力测试）或 M4（渐进式阅读）。
