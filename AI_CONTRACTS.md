...
- 本地校验分两级（§10 宁缺毋滥）：站不住的**单项删掉**——易错段不在词里、音节拼不回原词、易混词没给区别、例句不含目标词、联想过长；核心意思、钩子缺失/超长、钩子为通用建议或没有英文抓手、只重复词义、与旧钩子归一化后相同、自测问题缺失或泄露目标词时整条失败。
- 重新生成时把上一条的钩子和策略作为"要避开的"发出去。不说清楚避开什么，模型多半只是换几个字重说同一件事。
- 用户在拼写练习里的薄弱片段和真写错过的形式会进提示词（设计文档 §11 的闭环），但只带这一个词的记录，不带全局历史。

### 「怎么记」质量与首次学词（D-062）

- 批量新词 `memoryHintZh` 与独立 `memory_hook` 共用质量规则：推荐 20～90 字、最多两句，
  完整说清「英文抓手 → 当前词义或易错点的联系」。可靠构词、熟词对比、具体拼写规律、
  含关键英文的短场景均可；不再要求用 20 字压成口号，也不要求没见过的词能凭空猜中。
- 新词短提示以策略标签开头，人工联想必须说明性质；不把任意字母块当词根，不用冷僻同根词
  增加记忆负担。提示词同时给 unhappy / dessert / borrow / need 正例和空洞释义反例。
- 批量提示可留空。空白、超长和能识别的劣质提示清空并记录原因，保留其余合格的词卡；
  本地不能证明词源或联想有效性，不能把过了校验当作记忆效果保证。
- 独立提示推荐 20～90 字，本地容忍最短 4 字的有用短线索；长度按字符串字符计（含英文）。
  `recall_question` 必须能复用线索且不直接出现目标词，首屏提示用户遮住上文尝试回忆，不计分。
