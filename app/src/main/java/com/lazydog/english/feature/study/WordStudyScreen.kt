...
private fun StudyCardView(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            com.lazydog.english.feature.vocabulary.WordLearningContent(
                card = GeneratedWord(card.term, card.ipa, card.meaningZh, card.exampleEn, card.exampleZh,
                    pos = card.pos, forms = card.forms, collocations = card.collocations, memoryHintZh = card.memoryHintZh),
                revealed = revealed, isNew = card.isNew, itemId = card.itemId, facts = card.facts,
                memoryAssistance = card.memoryAssistance, onMemoryHint = onMemoryHint,
            )
        }
        Column(
