...
当前状态：M1～M5 代码完成（朗读提前，见 D-013）。能力小测（今日页横幅 / 设置 → 能力画像）：12 道自适应单选题（本地阶梯控制难度，AI 只出题），中断可恢复，测出 CEFR 等级 + 置信度，之后所有 AI 生成（新词/语法/阅读/点词点句解释）都按画像出题。AI 生成走 SSE 流式并显示进度。阅读页支持点词查词 / 点句讲解两种模式和全文朗读。今日页任务清单仍是示例数据（M6 收口）。下一步：M6（完整今日任务）。
