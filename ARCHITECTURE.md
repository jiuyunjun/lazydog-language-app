...
- 抽屉语音输入复用 `SpeechController.transcribeContinuously`；提问使用 Azure 中英自动检测，稳定草稿门槛设为 1，`recognizing` 草稿与 `recognized` 定稿实时合并回填。一句话停顿定稿后自动停止，用户也可再次点麦克风提前停止，最长收音 30 秒，识别结果不自动发送。情景演练继续使用 `transcribeOnce` 并限定英文识别。
