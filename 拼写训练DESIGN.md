...
## 0. 拼写分组口径修正（D-069，2026-09-07）

本节优先于下文旧的分组示例。「词块」在拼写训练中指书写分组，不等于词根、词缀或独立读音。
新词 chunks 可给 1～4 个非空片段，能拼回原词；可靠透明构词可分，不清楚则保留整词。
不再为凑两块拆单音节词，也不在本地用前后缀字符串和等长切分猜边界。
已有合法分组沿用；没有有效分组时返回整词，现有挖空/首字母提示继续按可用信息工作。
只有一组时接触卡隐藏分组区域；结构提示直说整词回想，不声称有多块。

`clever → clev / er` 是可用的书写分界，[韦氏词典](https://www.merriam-webster.com/dictionary/clever)
也标 clev·er；这不表示 clev 是词根、er 是后缀。真实读音用音标和整词播放。
UI 标为「拼写分组」并说明这一边界；只有已标易错段才高亮，不猜测中间块必然易错。
没有重写已有 chunks，不增加音节/构词分类字段、词典数据依赖或数据库迁移。
